...
gr_dat <-
  ...
  rename(
    fish_id = fish.id,
    birth_year = birth,
    month_capture = month.capture,
    year_capture = year.capture
  )

lagoonData <-
  fread("lagoon_data_sub.csv") %>%
  data.frame()

summerT <-
  fread("summer_temps.csv") %>%
  data.frame() %>%
  select(-c(1, season))
...
for (i in 1:ncol(y_vals))
  lines(rep(i, 2) - 1, y_vals[, i], lwd = 1.5, col = "red")
# ----------------------------------------------------
...
y_vals <- apply(mean_szB, 2, quantile, probs = c(0.025, 0.975))
for (i in 1:ncol(y_vals))
  lines(rep(i, 2) - 1, y_vals[, i], lwd = 1.5, col = "dark green")
...
for (i in 1:ncol(y_vals))
  lines(rep(i + 0.1, 2) - 1, y_vals[, i], lwd = 1.5, col = "red")
...
for (i in 1:ncol(y_vals))
  lines(rep(i - 0.1, 2) - 1, y_vals[, i], lwd = 1.5, col = "blue")
